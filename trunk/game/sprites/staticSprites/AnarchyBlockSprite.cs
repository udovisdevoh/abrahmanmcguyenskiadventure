...
namespace AbrahmanAdventure.sprites
{
    ...
    internal class AnarchyBlockSprite : StaticSprite, IBumpable
    {
        ...
        private bool isAllowNinja;

        private bool isAllowBodhi;
        ...
        internal AbstractSprite GetPowerUpSprite(PlayerSprite playerSprite, AbstractGameMode gameMode, Random random)
        {
            switch (BlockContent)
            {
                ...
                case BlockContent.RastaHat:
                    ...
                    else if (playerSprite.IsNinja && isAllowBodhi)
                    {
                    ...
                    else if ((playerSprite.IsDoped || playerSprite.IsRasta) && isAllowNinja)
                    {
                        BandanaSprite bandanaSprite = new BandanaSprite(XPosition, TopBound, random);
                        return bandanaSprite;
                    }
                    else if (!playerSprite.IsTiny && !playerSprite.IsBodhi && !playerSprite.IsNinja)
                    {
                ...
                case BlockContent.Vine:
                    ...
                    return vineSprite;
                default: //Peyote
                    if (gameMode.IsMushroomOverrideUpgrade || playerSprite.IsTiny)
                    {
                        MushroomSprite mushroomSprite = new MushroomSprite(XPosition, TopBound, random);
                        mushroomSprite.IsNoAiDefaultDirectionWalkingRight = playerSprite.IsTryingToWalkRight;
                        return mushroomSprite;
